...
import { Heart, Share2, MessageCircle, Send, User as UserIcon, Loader2, LinkIcon, Mail, CornerDownRight, Eye, EyeOff, ArrowLeft } from 'lucide-react';
import type { Post, Comment as CommentType, UserSummary, Category } from '@/types';
import { getPostById, likePost, sharePost, addComment, likeComment, getCategoriesWithCounts, getPosts } from '@/app/actions/post.actions';
...
export default function PostPage() {
  ...
  // State for sidebars
  const [popularCategoriesData, setPopularCategoriesData] = useState<(Category & { postCount: number })[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
        ...
        try {
            ...
            const enrichedCategories = staticCategories.map(sc => {
            ...
            setPopularCategoriesData(enrichedCategories);
            setTopAuthors(authors);
            setTrendingPosts(trending.posts);

        } catch (error) {
            console.error("Failed to fetch post or sidebar data:", error);
            toast({ title: "Error", description: "Could not load the page data.", variant: "destructive" });
        } finally {
            setIsLoading(false);
        }
    }
  ...
  useEffect(() => {
    if (user && post?.likedBy) {
      setIsPostLikedByCurrentUser(post.likedBy.includes(user.id));
    } else {
    ...
    try {
        ...
        if (updatedPostFromServer) {
         ...
         setPost(prevPost => {
            ...
            if (!originalComment) return prevPost; 

            const revertedComments = prevPost.comments.map(c => {
                if (c.id === commentId) {
                    return { ...c, likes: originalComment.likes, likedBy: originalComment.likedBy };
                }
                return c;
            });
  ...
  if (isLoading) {
    return (
        <div className="flex-grow flex items-center justify-center h-[calc(100vh-130px)]">
        ...
        <div className="flex justify-center gap-8">
          ...
          <section className="flex-grow max-w-2xl flex flex-col gap-4">
            <div className="w-full">
                <div className="mb-4">
                    <Button variant="ghost" size="sm" className="h-8 w-auto px-2 rounded-full" onClick={() => router.back()}>
          ...
          {/* Right Sidebar */}
          <aside className="hidden lg:block w-[316px] shrink-0">
            <div className="sticky top-20 space-y-6 h-[calc(100vh-6rem)] overflow-y-auto">
